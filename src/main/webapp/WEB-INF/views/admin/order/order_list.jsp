                  ...
                  <tbody>
                    ...
                    <c:forEach items="${order_list}" var="orderVO">
                      <tr>
                        ...
                        <!-- 주문관리 -->
                        <!-- 버튼에 상품코드 정보를 숨겨 넣는다. 서버에 요청 작업에 필요-->
    ...
    <tbody>
      ...
      {{#each .}}
        <tr>
          ...
          <!-- 수량 -->
          <th style="width: 10%">{{ord_amount}}</th>
          <!-- 주문금액 -->
          <th style="width: 20%">{{prd_tot_price}}</th>
          <!-- 비고 -->
          <th style="width: 10%">
            <button type="button" name="btn_orderProductDel" data-prd_no="{{prd_no}}" class="btn btn-link">상품삭제</button> <!-- 주문상세-상품삭제 용도의 상품코드 숨겨넣음 -->
          </th>
        </tr>
      {{/each}}
    </tbody>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">주문 상세내역</h5>
...
<script>
  ...
  $(document).ready(function(){
    ...
    $("div#modalDetailView").on("click", "button[name='btn_orderProductDel']", function(){
      ...
      let prd_no = $(this).data("prd_no");  // 상품번호
      let ord_no = $(this).parent().parent().find("input[name='ord_no']").val();  // 주문번호
      ...
      $.ajax({
        ...
        type: 'post',
        data: {prd_no:prd_no, ord_no:ord_no},
        dataType: 'text',
        success: function(result){
          if(result == 'success'){
            alert("주문상품이 삭제되었습니다.");
